 ...
 **/
public class WxNimbusUserInfoResponseClient {
    static Logger LOG = LoggerFactory.getLogger(WxNimbusUserInfoResponseClient.class);
    Gson gson = new Gson();
    ...
    <T> T getUserInfoResponse(WxOAuth2UserRequest userInfoRequest, Class<T> returnType) throws OAuth2AuthenticationException {
        ClientHttpResponse userInfoResponse = this.getUserInfoResponse(
                userInfoRequest.getClientRegistration(),
                userInfoRequest.getAccessToken(),
                userInfoRequest.getOpenid());
        try {
    ...
    <T> T getUserInfoResponse(WxOAuth2UserRequest userInfoRequest, ParameterizedTypeReference<T> typeReference) throws OAuth2AuthenticationException {
        ClientHttpResponse userInfoResponse = this.getUserInfoResponse(
                userInfoRequest.getClientRegistration(), userInfoRequest.getAccessToken(), userInfoRequest.getOpenid());
        try {
    ...
    private ClientHttpResponse getUserInfoResponse(ClientRegistration clientRegistration,
                                                   OAuth2AccessToken oauth2AccessToken, String openid) throws OAuth2AuthenticationException {
        ...
        WxUserInfoRequest userInfoRequest = new WxUserInfoRequest(userInfoUri, accessToken, openid);
        LOG.info("getUserInfoResponse.userInfoRequest:" + gson.toJson(userInfoRequest));
        HTTPRequest httpRequest = userInfoRequest.toHTTPRequest();
        LOG.info("getUserInfoResponse.httpRequest:" + gson.toJson(httpRequest));
        ...
        try {
            ...
            LOG.info("getUserInfoResponse.httpResponse:" + gson.toJson(httpResponse));
        ...
        if (httpResponse.getStatusCode() == HTTPResponse.SC_OK) {
            return new WxNimbusUserInfoResponseClient.NimbusClientHttpResponse(httpResponse);
        }
